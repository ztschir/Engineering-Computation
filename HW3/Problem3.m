...
standerrQuad = sqrt(sum((polyval(quad,t) - c).^2))
standerrCubic = sqrt(sum((polyval(cubic,t) - c).^2))

avgerrLin = standerrLin/length(t)
avgerrQuad = standerrQuad/length(t)
avgerrCubic = standerrCubic/length(t)

relativeerrLin = standerrLin/sqrt(sum(c.^2))
relativeerrQuad = standerrQuad/sqrt(sum(c.^2))
relativeerrCubic = standerrCubic/sqrt(sum(c.^2))